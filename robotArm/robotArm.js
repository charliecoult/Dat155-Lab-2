...
var LOWER_FINGERS_WIDTH      = 0.2;
var LOWER_FINGERS_HEIGHT     = 2.0;
var UPPER_FINGERS_WIDTH      = 0.2;
var UPPER_FINGERS_HEIGHT     = 2.0;
...
function lowerFingers()
{
    let s = scale(LOWER_FINGERS_WIDTH, LOWER_FINGERS_HEIGHT, LOWER_FINGERS_WIDTH);
    let instanceMatrix = mult(translate( 0.0, 0.5 * LOWER_FINGERS_HEIGHT, 0.0 ), s);
...
function upperFingers() {
    let s = scale(UPPER_FINGERS_WIDTH, UPPER_FINGERS_HEIGHT, UPPER_FINGERS_WIDTH);
    let instanceMatrix = mult(translate( 0.0, 0.5 * UPPER_FINGERS_HEIGHT, 0.0 ), s);
...
render = function () {
    ...
    modelViewMatrix = mult(modelViewMatrix,rotateY(theta[Claw_joints]));

    let stack = [];
    stack.push(modelViewMatrix);

    modelViewMatrix = mult(modelViewMatrix, translate(0.0, UPPER_ARM_HEIGHT, 0.0));
    modelViewMatrix = mult(modelViewMatrix, rotate(theta[UpperFingers], 0, 0, 1));

    lowerFingers();

    modelViewMatrix = mult(modelViewMatrix, translate(0.0, LOWER_FINGERS_HEIGHT, 0.0));
    modelViewMatrix = mult(modelViewMatrix, rotate(theta[LowerFingers], vec3(0, 0, 1)));
